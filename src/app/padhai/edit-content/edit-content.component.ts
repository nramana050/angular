...
import { AppConfirmService } from 'src/app/common-files/app-confirm/app-confirm.service';
import { SnackBarService } from 'src/app/common-files/service/snack-bar.service';
...
import { AppInsightsService } from 'src/app/common-files/service/app-insights.service';
import { FileUploadService } from 'src/app/common-files/file-upload/file-upload.service';
import { LearningOutcomesComponent } from '../learning-outcomes/learning-outcomes.component';
// import * as Editor from '@equation/padhai-ck-editor/ckeditor';
import {colors} from 'src/assets/padhaiFontColorConfig/FontColorConfig';
...
export class EditContentComponent implements OnInit, OnDestroy  {
  ...
  constructor(
   ...
   setTimeout(() => {
    ...
    this.padhaiService.newImageUploadedSubject.subscribe(file => {
    ...
    this.padhaiService.courseOutlineSuccessSubject.pipe(debounceTime(300),distinctUntilChanged()).subscribe(data => {
  ...
  canDeactivate(): Observable<boolean> | boolean {
    this.unsubscribeIntervals();
    this.padhaiService.dataServiceSubject.next();
  ...
  async getCourseoutlineDetails() {
    ...
    switch (courseStatus) {
      ...
      case 'outlinefailed':
        ...
        this.padhaiService.dataServiceSubject.next({ img: this.courseImage, stage: this.stage ,course:courseAssetData});
        this.padhaiService.courseImageSubject.next({currentStatus : this.stage})
        break;
  ...
  async editLesson(node) {
    ...
    this.padhaiService.newImageUploadedSubject.subscribe(data => isNewImageUploaded = data);  
      ...
      this.showLoader = true;
        ...
        if(this.courseStage == 'Lesson'){
          ...
          if(this.stage != 'assetgenerated' || isNewImageUploaded){
            this.padhaiService.newImageUploadedSubject.next(false); 
            await this.getLessonDetails(this.activeLangauge);
          }
        }
  ...
  getCourseStatusData() {
    ...
    return new Promise(resolve => {
      this.padhaiService.getCourseStatus(this.courseId).subscribe(res => {
        ...
        this.padhaiService.courseImageSubject.next({currentStatus : this.currentStage.replace(/\s/g, '')})
      ...
      if(result){
        this.padhaiService.approveCourse(this.courseId).subscribe(res => {
          ...
          if (res.responseObject.isCourseApproved) {
            ...
            this.padhaiService.dataServiceSubject.next({ img: this.courseImage, stage: this.stage });
            this.snackBarService.success(res.message.applicationMessage);

...
export class CourseSetupComponent implements OnInit {
  ...
  dataSource = new MatTableDataSource<ICourseSetup>();
  displayedColumns: string[] = ['courseName', 'courseCode', 'courseType','glh','action'];
  ...
  resolveUsers(filterBy) {
    ...
    this.courseSetupService.findAllPaginated(`${this.sortColumn},${this.sortDirection}`, currentPageIndex, this.pageSize, filterBy)
      .subscribe(resp => {
  ...
  onDelete(id) {
    ...
    dialogRef.subscribe(result => {
      if (result) {
        this.courseSetupService.deleteCourse(id).subscribe(response => {

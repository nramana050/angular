...
import { PLAN_INDUCTION_VALIDATION_CONTEXT } from 'src/app/features/captr-learners/view-captr-learner/plan-induction/plan-induction.validation';
import { ViewAssessmentPopUpComponent } from 'src/app/features/captr-learners/view-captr-learner/plan-induction/view-assessment-pop-up/view-assessment-pop-up.component';
...
@Component({
  selector: 'app-action-plan-induction',
  templateUrl: './action-plan-induction.component.html',
  styleUrls: ['./action-plan-induction.component.scss']
})
export class ActionPlanInductionComponent implements OnInit, OnDestroy {
  ...
  checkAnsPath: './clink-learners/profile/check-answer';
  ...
  questionDisplay() {
    ...
    this.planInductionService.getAnsweredQuestion(this.userId).subscribe(data => {
      ...
      if (this.actionPlanCreatedFlag === true) {
        this.router.navigate([this.profileUrl + '/profile/check-answer'], { queryParams: { id: this.userId, name: this.fname } });
      } else {
        this.router.navigate([this.profileUrl + '/profile/action-plan'], { queryParams: { id: this.userId, name: this.fname } });
      }
    })
  }
  ...
  goToProfile(){
    this.router.navigate([this.profileUrl + '/profile'], { queryParams: { id: this.userId, name: this.fname } });

...
export class EditEnrolmentComponent implements OnInit, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params: any) => {
      ...
      this.captrLearnersService.getLearnerEnrolmentDetails(this.id, this.did).subscribe(enrolmentDetails => {
        this.cohort = enrolmentDetails.programmeDeliverySeq
  ...
  getLastAttendanceDate() {
    this.captrLearnersService.getLastAttendedDate(this.did, this.id).subscribe(data => {
      if (data) {
  ...
  navigateToWorkshopAttendance() {
    this.route.queryParams.subscribe((params: any) => {
      this.router.navigate(['/captr-learner/enrolment-details'], { queryParamsHandling: 'merge', queryParams: { id: params.id, name: params.name } });
    });
  }
  ...
  onSubmit() {
    ...
    if (!(this.programmeStatus === 'Withdrawn') && this.programmeStatus !== 'Finished - Not Completed' && this.programmeStatus !== 'Completed') {
      const dialogRef = this.appConfirmService.confirm({
        ...
        message: `Once a Participant has been Withdrawn from a Programme, this cannot be un-done. Are you sure you want to Withdraw this Participant from the Programme?`,
    ...
    if (this.programmeStatus === 'Finished - Not Completed') {
      const dialogRef = this.appConfirmService.confirm({
        ...
        message: `Once the programme has been marked as completed, you will no longer be able to change the status or actual end date. Are you sure you want to mark this Participant as having completed the Programme?`,
        okButtonLabel: 'Yes - Continue'
      });
      this.saveEnrolmentForProgrammeStatus(dialogRef,payload);
    }
  ...
  saveEnrolment(payload) {
    this.captrLearnersService.saveEnrolment(payload).subscribe(rsp => {
      this.router.navigate(['captr-learner/enrolment-details'],{ queryParamsHandling: 'merge'});
      if(payload.programStatus === 2){
        this.snackBarService.success("Participant withdrawn successfully");
      }
      else if(payload.programStatus === 4) {
        this.snackBarService.success("Participant program status has been changed to completed");
      }
    },
  ...
  updateEnrolment(payload) {
    this.captrLearnersService.updateEnrolment(payload).subscribe(rsp => {
      this.router.navigate(['captr-learner/enrolment-details'], { queryParamsHandling: 'merge', queryParams: { id: rsp.learnerId, name: this.fullName, did: rsp.programmeDeliveryId } });
      this.snackBarService.success("Participant enrolment details updated successfully");
    },
      error => this.snackBarService.error(error.error.applicationMessage)
    )
  }

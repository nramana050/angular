...
import { ErrorMessage } from '../../plan-content-card/track-tab/file-validation.service';
...
@Component({
  selector: 'app-add-or-edit-case-notes',
  templateUrl: './add-or-edit-case-notes.component.html',
  styleUrls: ['./add-or-edit-case-notes.component.scss'],
  providers: [
...
export class AddOrEditCaseNotesComponent implements OnInit, OnDestroy, AfterViewChecked {
    ...
    if (this.updateFlag) {
      this.learnersService.getUserCaseNote(this.caseNoteId).subscribe(data => {
        ...
        if(this.workerList.filter(data1=>data1.id === data.noteSenderId).length==0 ){          
          this.caseNoteForm.get('noteSenderId').patchValue(null)

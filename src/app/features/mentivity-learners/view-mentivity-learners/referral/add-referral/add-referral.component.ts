...
export class AddReferralComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    this.manageUsersService.getFilterUserList(4).subscribe(userData=>{
    ...
    const  userId = JSON.parse(atob(localStorage.getItem('token').split('.')[1])).userId;
    this.manageUsersService.getFilterUserList(9).subscribe(userData=>{
      const result =  userData.filter(user => user.id == userId )
      if(result.length>0){  
        const value =[{"id":result[0].id , "userFullName":result[0].firstName + ' ' + result[0].lastName}]
        this.workerList = value;
  ...
  addReferral() {
    ...
    this.referrralService.addReferral(payload).subscribe(result => {
      ...
      this.router.navigate(['/mentivity-learner/referral'], { queryParams: { id: this.userId, name: this.name } })
    },
      error => {

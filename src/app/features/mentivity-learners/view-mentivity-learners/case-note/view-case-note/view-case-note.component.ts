...
@Component({
  ...
  styleUrls: ['./view-case-note.component.scss']
})
export class ViewCaseNoteComponent implements OnInit {
  ...
  ngOnInit() {
    this.mentivityService.viewUserCaseNote(this.caseNoteId).subscribe(data => {
  ...
  onExitClicked() {
    this.router.navigate([this.profileUrl +'/case-note'], { queryParams:  {id: this.UserId, name: this.fName}}); 
    }
}

...
export class EditUserComponent implements OnInit {
  ...
  async getSelectedOrg() {
    if (this.routeIntent === 'editUser') {
      ...
      await this.manageUsersService.getUserDetails(this.userid).toPromise().then(userData => {
        
        this.userOrganizations = userData.orgList;
  ...
  resolveOrganizations() {   
    this.manageUsersService.getOrganizations().subscribe(data => {
    ...
    if(this.organizations?.length>1)
      {                
        this.userForm.get('orgList').addValidators(Validators.required);
        this.userForm.get('orgList').updateValueAndValidity();
      }
  ...
  createUserForm() {
    this.userForm = this.fb.group({
      ...
      userTypeId: null,
      orgList: this.fb.array([])
    });

    if (this.routeIntent === 'editUser') {
      this.activatedRoute.params.subscribe((params: any) => {
        this.manageUsersService.getUserDetails(params.id).subscribe(userData => {
         ...
         // this.userForm.controls['orgList'].setValue();
          this.updateOrganizations(userData.orgList)
  ...
  onSaveNewUser(userForm) {
    ...
    if(payload.orgList.length==0)
    {
      payload.orgList =this.organizations;           
    }
  ...
  checkLicences() {
    this.manageUsersService.getValidLicences(this.userType).subscribe(data => {
      if (data.isAuthorisedLicence) {
        this.router.navigateByUrl('/manage-users/new-user');
      }
      else {
  ...
  initAllLists() {
    this.manageUsersService.getAllocatedRefData().subscribe(data =>{

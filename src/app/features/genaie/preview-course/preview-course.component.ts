...
export class PreviewCourseComponent implements  OnInit, OnDestroy {
  ...
  h5pLibrariesPath = `${environment.cdnUrl}/genaie/h5p_libraries`;
  frameBundlePath = `${environment.cdnUrl}/genaie/h5p_libraries/main/frame.bundle.min.js`;
  assetH5p;
  showH5p = false;
  ...
  getLessonDetails(language:string){
    this.genaieService.getLessonRequest(this.courseId,language).subscribe( res=>{
  ...
  getTransaltedLanguageListForCourse(courseId: any) {
    this.genaieService.getTranslatedLanguageList(courseId).subscribe(res => {

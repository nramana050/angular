...
export class ContentGeneratorComponent implements OnInit, AfterViewInit {
  ...
  goToEdit( element ){
    ...
    this.router.navigate([`genaie/edit-content/${element.id}`],{
  ...
  previewCourse( element ){
    localStorage.setItem('courseName',element.topic)
    this.router.navigateByUrl(`genaie/preview-content/${element.id}`);
  }
  ...
  getLicenceDetailsData(){
    this.genaieService.getLicenceDetails().subscribe(data=>{

...
export class LearningOutcomesComponent implements OnInit, OnChanges {
 ...
 async ngOnInit() {
  ...
  getCourseStatus(){
    this.genaieService.getCourseStatus(this.courseId).subscribe(statusData=>{      
      if(statusData){
 ...
 async performActionOnOutcomeStatus(){
  ...
  getCourseData(){
    return new Promise(resolve=>{
      this.genaieService.getLessonRequest(this.courseId, this.activeLanguage || 'ENGLISH').subscribe(courseData=>{
  ...
  saveLearningOutcomes(){
    ...
    this.genaieService.saveLearningOutcomesAsDraft(this.courseDetails).subscribe(res=>{

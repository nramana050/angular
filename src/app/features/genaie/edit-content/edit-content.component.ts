...
export class EditContentComponent implements OnInit, OnDestroy, CanComponentDeactivate  {
  ...
  public onReady(editor) {
    editor.plugins.get('FileRepository').createUploadAdapter = (data) => {
      return new MyUploadAdapter(data, this.courseId, this.genaieService, this.snackBarService);
    };
  }
  ...
  getExportToMenuTypes() {
    this.genaieService.getExportType().subscribe(data => {
  ...
  async getCourseoutlineDetails() {
    ...
    switch (courseStatus) {
      ...
      case 'outlineinprogress':
        if(window.location.href.includes('/genaie/edit-content')){
          setTimeout(() => {
            this.getCourseoutlineDetails();
            }, 2000);
        }
    ...
    // this.genaieService.getCoursesRequest(this.courseId).subscribe(async res => {
    ...
    //     }
    //   }else{
    //     if (courseStatus == 'outlineinprogress' || res && res.courseOutLine == null && window.location.href.includes('/genaie/edit-content')) {
    //       setTimeout(() => {
    //       this.getCourseoutlineDetails();
    //       }, 2000);
    //     } else {
    //       if (res.asset == null && window.location.href.includes('/genaie/edit-content')) {
    //         setTimeout(() => {
    //           this.getCourseoutlineDetails();
    //         }, 2000);
    //       }
  ...
  getLessonDetails(language?, isFirstTimeLoad?, isLanguageChange = false){
    return new Promise(resolve=>{
      this.genaieService.getLessonRequest(this.courseId, language ? language : this.defaultLanguage).subscribe(async res=>{
  ...
  async saveDraft(courseData) {
    ...
    this.genaieService.saveCourseAsDraft(this.courseDetails).subscribe(res => {
  ...
  async saveGenerate(courseData) {
    ...
    this.disableLessonButton = true;
    await this.patchFromData(courseData);
    this.genaieService.saveAndGenerateCourse(this.courseDetails).subscribe(res => {
      // this.router.navigateByUrl('/genaie');
      this.isOutlineFormEdited = false;
      this.editCourseMode = false;
      this.showCourseContainer = false;
  ...
  isEdited(key: string, i: number, j: number): boolean {
   ...
   async saveLessonForm() {
    ...
    this.genaieService.saveLessonAsDraft(payload).subscribe(async res => {
      // this.setDefaultStructureForH5p();
  ...
  async generateAsset(outcomesData) {
    ...
    this.tempLessonsData['outcomes'] = outcomesData;
     await this.patchStatusToLessonData();
      this.genaieService.assetGeneration(this.tempLessonsData).subscribe(res => {
  ...
  getLessonStatus(stageType,expand, isLanguageChange = false) {
    if(window.location.href.includes("/edit-content/")){
      ...
      if(isLanguageChange){
        ...
        this.genaieService.getCourseLessonStatus(this.courseId, stageType).subscribe(statusData => {
  ...
  getCourseStatusData() {
    ...
    return new Promise(resolve => {
      this.genaieService.getCourseStatus(this.courseId).subscribe(res => {
  ...
  selectExportType(exportFile) {
    ...
    if (payload.exportType !== exportTypeEnum.MOODLE) {
      this.genaieService.getLessonRequest(this.courseId, this.activeLangauge).subscribe(async res=>{
      ...
      this.genaieService.export(payload).subscribe(res => {
        let BlobPath=  this.getBlobPath(res,payload.exportType) ;
        let blobUrl = environment.cdnUrl +'/genaie/'+ BlobPath;
      ...
      this.genaieService.export(payload).subscribe(result => {
...
getTransaltedLanguageListForCourse(courseId: any) {
  this.genaieService.selectedLanguagesData(courseId).subscribe(res => {
...
retryLessonAndAsset() {
  this.genaieService.getCourseStatus(this.courseId).subscribe(async res => {
    if(res.courseStatus == "Outline" && res.outlineStatus == "Failed"){
      //perform retry for outline
      let payload = {
        courseRequestId: res.id
      }
      this.genaieService.retryCourseStage(payload).subscribe(data => {
        this.showFailedStatus = false;
      ...
      this.selectStage();
      this.genaieService.retryCourse(payload, res.courseStatus).subscribe(res => {
        this.showFailedStatus = false;
        this.getCourseStatusData();
...
retryFailedLanguages(){
  ...
  this.genaieService.retryCourse(payload,'Translate').subscribe(res=>{
...
getSelectedLanguagesData(){
  this.genaieService.selectedLanguagesData(this.courseId).subscribe(lang=>{
    this.languages = lang;  
  })
}
  ...
  if(!this.licenceExceeded){
    ...
    approveDialog.subscribe(result=>{
      if(result){
        this.genaieService.approveCourse(this.courseId).subscribe(res => {
  ...
  checkCourseApproveStatus() {
    this.genaieService.getCourseApproveStatus(this.courseId).subscribe(res => {
  ...
  async generateOutcomes(){
    ...
    if (this.allH5pSelectedFlag && this.allH5pSelectedFlag != undefined) {
      ...
      this.genaieService.saveLessonAsDraft(lessonDataPayload).subscribe(async res => {
        let payload = {"courseRequestId" : this.courseId};
        this.genaieService.generateLearningOutcome(payload).subscribe(res=>{
          this.snackBarService.success(res.message.applicationMessage);
          this.getCourseStatusData();
          this.resetFlags();
          this.editLessonMode = false;
  ...
  retryLearningOutcome(){
    ...
    this.genaieService.retryCourse(payload,'Outcome').subscribe(res=>{
      this.getCourseStatusData();
      this.snackBarService.success(res.message.applicationMessage);
    },err=>{
      this.snackBarService.error(err.error.applicationMessage);
  ...
  getH5pTypeList() {
    this.genaieService.getH5pTypeList().subscribe(data => {
      this.h5pTypes = data;
    });
  }
  ...
  async getLicenceDetails(type: string) {
    await this.genaieService.getLicenceDetails().toPromise()
      .then(res => {
        ...
        this.licenceExceeded = true;
        this.appConfirmService.confirm({ title: 'Action required', message: `Sorry, you don't have GENAIE licence to perform this action. Please contact your administrator.`, showOkButtonOnly: true });
